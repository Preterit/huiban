 ...
 * @author admina
 */
public class Statement3Activity extends Statement1Activity implements OnClickListener {
    ...
    public void commit() {
        if (TextUtils.isEmpty(et_1.getText().toString().trim())) {
            T.showShort(this, "请输入本月完成工作的内容");
            return;
        }
        if (TextUtils.isEmpty(et_3.getText().toString().trim())) {
            T.showShort(this, "请输入下月计划");
            return;
        }
        if (shenpi==false) {
...
//        }
        ...
        Utils.doPost(LoadingDialog.getInstance(Statement3Activity.this), Statement3Activity.this, url, params, new Utils.HttpCallBack() {
            @Override
            public void success(final JsonBean bean) {
                if ("200".equals(bean.getCode())) {
                    runOnUiThread(new Runnable() {
                        public void run() {
                            T.showShort(Statement3Activity.this,
                                    bean.getMsg());

                            Statement3Activity.this.finish();
            ...
            public void failure(String msg) {
                T.showShort(Statement3Activity.this, msg);
            }
