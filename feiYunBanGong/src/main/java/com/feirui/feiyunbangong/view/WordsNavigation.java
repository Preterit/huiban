...
public class WordsNavigation extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (itemWidth < itemHeight) {
            wordsPaint.setTextSize(itemWidth / 2);// 畫筆根據寬度來確定；
        } else {
            wordsPaint.setTextSize(itemHeight / 2);// 畫筆根據寬度來確定；
        }
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            if (touchIndex == i) {
                ...
                canvas.drawCircle(itemWidth / 2, itemHeight / 2 + i
                        * itemHeight + 2, itemWidth / 4, bgPaint);
